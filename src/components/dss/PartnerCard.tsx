...
export const Portrait: React.FC<{ partner: PartnerView; className?: string }> = ({ partner, className }) => (
    <div className={`relative aspect-[4/5] overflow-hidden rounded-xl bg-brand-navy-dark border border-brand-gold/30 ${className ?? ''}`}>
        {typeof partner.portrait === 'string' ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={partner.portrait} alt={partner.name} className="w-full h-full object-cover object-top" />
        ) : partner.portrait ? (
            <Image src={partner.portrait} alt={partner.name} className="w-full h-full object-cover object-top" />
        ) : (
...
const PartnerCard: React.FC<{ partner: PartnerView; full?: boolean }> = ({ partner, full }) => (
    <article className="tile p-6 md:p-7 flex flex-col">
        ...
        <div className="flex gap-3 mt-6 pt-5 border-t border-brand-gold/15">
            ...
            {partner.email && <span className="self-center text-sm text-white/45">{partner.email}</span>}
        </div>
    </article>
);
