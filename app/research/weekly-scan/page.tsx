...
export default async function WeeklyScanPage({ searchParams }: { searchParams?: { territory?: string } }) {
  ...
  const scans = filter ? all.filter((s) => s.territory === filter) : all;
  ...
  return (
    <>
      ...
      <Section>
        ...
        {hasSamples && (
          <div className="tile-ivory p-4 mb-8 text-sm">
            <strong>Prototype note.</strong> Entries marked “sample” are placeholders showing the structure of a scan entry. They are replaced by real scans before launch.
          </div>
        )}
          ...
          {scans.map((e) => (
            <Link key={e.slug} href={`/research/weekly-scan/${e.slug}`} className="group tile p-7 hover:border-brand-gold/60 transition-colors">
              <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-3">
                ...
                <span className="text-white/45 text-sm">Week of {e.weekOf}</span>
