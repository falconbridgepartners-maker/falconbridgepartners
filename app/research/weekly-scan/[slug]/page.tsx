...
import { getAllScans, getScanBySlug, territoryName } from '@/lib/content';
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!e) return {};
  return { title: `${territoryName[e.territory] ?? e.territory} · Week of ${e.weekOf} — Weekly Scan — FalconBridge Partners`, description: e.question };
}

export default async function ScanEntryPage({ params }: { params: { slug: string } }) {
  const e = await getScanBySlug(params.slug);
  if (!e || e.draft) notFound();
  const svc = services.find((s) => s.key === e.service);
  return (
    <>
      <PageHero eyebrow={`Weekly Scan · ${territoryName[e.territory] ?? e.territory} · Week of ${e.weekOf}`} title="A signal, and the question it raises" governing={e.question} intro={e.signal}>
      ...
      <Section>
        ...
        {svc && (
          <div className="mt-8">
            <NextLink href={`/decision-support-system/${svc.slug}`} label={`${svc.acronym} · ${svc.name}`} sub="The service closest to this question." />
          </div>
        )}
